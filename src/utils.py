...
def setup_wandb(
    ...
    # 如果是恢复运行，先初始化 wandb
    if hasattr(config.wandb, 'id') and config.wandb.id:
        run = wandb.init(
        ...
        # 保持原始配置，只更新 wandb 相关的配置
        # # 直接更新 config.wandb 的 __dict__
        # config.wandb.__dict__.update({
        #     k: v for k, v in run.config.items() 
        #     if k in config.wandb.__dict__ and k not in ['id', 'resume']
        # })
        # 遍历并更新 wandb 配置
        for k, v in run.config.items():
            if hasattr(config.wandb, k) and k not in ['id', 'resume']:
...
def get_datamodule(
    ...
    additional_transforms = torchgeo.transforms.AugmentationSequential(
        ...
        kornia.augmentation.Resize(
            (config.data.img_size, config.data.img_size*2), keepdim=True
        ),
        data_keys=data_keys,
    )

    print(f"band_idx: {band_idx}")
    print(f"datamodule.mean shape: {datamodule.mean.shape}")
    print(f"datamodule.std shape: {datamodule.std.shape}")
    ...
    print("config.data.in_chans: ", config.data.in_chans)
