...
def adapt_weights_to_mae(model, checkpoint):
        """Adapt DINOv2 weights to MAE model"""
        model_dict = model.state_dict()
        
        # Extract encoder weights
        dino_dict = {k: v for k, v in checkpoint.items() if k in model_dict}
        
        # Handle potential differences in patch embedding
        if "patch_embed.proj.weight" in dino_dict:
            pe_weight = dino_dict["patch_embed.proj.weight"]
            if model_dict["patch_embed.proj.weight"].shape != pe_weight.shape:
                # Adjust number of input channels if needed
                if model_dict["patch_embed.proj.weight"].shape[1] != pe_weight.shape[1]:
                    dino_dict["patch_embed.proj.weight"] = upscale_patch_embed(
                        model_dict["patch_embed.proj.weight"].shape[1],
                        pe_weight
                    )
        
        # Handle position embeddings
        if "pos_embed" in dino_dict:
            if model_dict["pos_embed"].shape != dino_dict["pos_embed"].shape:
                # print("dino_dict:", dino_dict["pos_embed"])
                dino_dict["pos_embed"] = src.pos_embed.resample_abs_pos_embed(
                    dino_dict["pos_embed"],
                    new_size=(model.patch_embed.img_size),
                    old_size=(518, 518)
                )

        if "mask_token" in dino_dict:
            mask_token = dino_dict["mask_token"]
            if model_dict["mask_token"].shape != mask_token.shape:
                # 添加一个维度使形状从 [1, 384] 变为 [1, 1, 384]
                dino_dict["mask_token"] = mask_token.unsqueeze(1)
                
        print(f"Loading pretrained DINOv2 weights")
        missing, unexpected = model.load_state_dict(dino_dict, strict=False)
...
def get_dinov2_mae(
    ...
    in_chans=3,
    img_size=224,
    patch_size=14,  # DINOv2 默认patch_size
):
    if model_type == "mae":
        ...
        if pretrained:
            ...
            # 处理权重加载和调整
            adapt_weights_to_mae(model, checkpoint)
...
def get_model(**hparams):
    ...
    * forward img_size and patch_size args to non sat_mae models
    """
    if hparams["model"] == "dinov2_mae":  
        model = get_dinov2_mae(
            hparams["model_type"],
